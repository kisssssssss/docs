		...
		const Node = (
			<div key={key} id={key}>
				<p className='text-lg font-bold dark:text-gray-200/85'>
					{keyMap[key]}
					<span className='mx-2 text-base text-[#00000073] dark:text-gray-300/30 font-semibold'>数量：{list[key].length}</span>
				</p>
...
const ModelList = memo(({ enable, darkMode }) => {
	...
	const [renderer, setRenderer] = useState(<ModelListSkeleton darkMode={darkMode} />);
